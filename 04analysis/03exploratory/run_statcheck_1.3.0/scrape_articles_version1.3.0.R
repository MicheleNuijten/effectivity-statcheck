...
# load packages
install.packages('statcheck') # version 1.3.0 on CRAN
library(statcheck)
...
source("03data/xx_scrape_html_files_failsafe.R")
...
# remove rows with missing sources
result <- result[!is.na(result$source), ]

write.table(final_result, paste0("04analysis/03exploratory/", Sys.Date(), "scraped_articles_1.3.0.txt"))
